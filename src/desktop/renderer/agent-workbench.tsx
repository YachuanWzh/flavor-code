...
export function AgentWorkbench({ snapshot, onClose, onError, onCompose }: {
  ...
  return <section className="agent-workbench manager-view">
    <header className="manager-header workbench-header"><div><button onClick={onClose}>‹</button><div><small>FLAVOR DESKTOP 1.3.4</small><h2>Agent 工作台</h2></div></div><span>{snapshot.activeSession?.environment === "worktree" ? "隔离工作树" : "本地检出"}</span></header>
