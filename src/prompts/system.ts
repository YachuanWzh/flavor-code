...
function toolsSection(toolNames: ReadonlySet<string>): string {
  const rules: string[] = [];
  addToolRule(rules, toolNames, "Read", "Use `Read` to inspect files; read enough surrounding context to understand code before changing it. For large files pass `startLine`/`endLine` to fetch only the region you need, and never re-read a file whose unchanged content is already in context — a `[Duplicate read suppressed]` result means the content is still available in the conversation. Only read files inside the workspace. When you are not certain a path exists, find it with a search first — never construct paths from memory (the real entry may be `src/cli.tsx` while you guessed `src/cli.ts`).");
  addToolRule(rules, toolNames, "Write", "Use `Write` to create a file only when a new file is necessary.");
  addToolRule(rules, toolNames, "Edit", "Use `Edit` for precise replacements in existing files. Before editing, read the file and copy `oldText` verbatim from its actual current content — exact characters matter, including quote style (full-width vs ASCII) and version strings. Include enough surrounding lines to make the match unique.");
  addToolRule(rules, toolNames, "ApplyPatch", "Use `ApplyPatch` only when an edit genuinely spans multiple regions of one file. Numbered and bare `@@` hunk headers are accepted; hunks may relocate only when their exact context has one unique match anywhere in the file. Prefer `Edit` for a single small replacement — it is easier to get right.");
  addToolRule(rules, toolNames, "Glob", "Use `Glob` to find files by path pattern instead of shell directory crawling. Confirm the exact path before reading, searching, or editing; the search scope is limited to the workspace, so never pass absolute paths outside it. Set `includeIgnored: true` only when the user explicitly asks to inspect ignored files or an ignored path is required for the task.");
  addToolRule(rules, toolNames, "Grep", "Use `Grep` to search file contents instead of shell text-search commands. Set `includeIgnored: true` only when the user explicitly asks to inspect ignored files or an ignored path is required for the task.");
  addToolRule(rules, toolNames, "Shell", "Use `Shell` for builds, tests, Git inspection, and commands without a dedicated tool. Pass only the executable name or path in `command`, and put each logical argument in its own `args` item without shell quotes. Put pipes, redirects, and operators in `command` only when shell syntax is actually required. Prefer `Glob` and `Grep` over platform-specific directory and text-search commands. Keep commands scoped, non-interactive, and easy to audit.");
  ...
  addToolRule(rules, toolNames, "Skill", "Use `Skill` whenever an active skill requires or references another skill. Load the required skill before continuing, pass arguments when the nested workflow has its own task input, and follow the returned instructions completely.");
