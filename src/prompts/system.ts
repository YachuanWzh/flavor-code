...
function toolsSection(toolNames: ReadonlySet<string>): string {
  ...
  addToolRule(rules, toolNames, "ApplyPatch", "Use `ApplyPatch` only when an edit genuinely spans multiple regions of one file; hunks may relocate only when their exact context has one unique nearby match. Prefer `Edit` for a single small replacement — it is easier to get right.");
