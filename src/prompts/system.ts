...
function toolsSection(toolNames: ReadonlySet<string>): string {
  const rules: string[] = [];
  addToolRule(rules, toolNames, "Read", "Use `Read` to inspect files; read enough surrounding context to understand code before changing it. Only read files inside the workspace. When you are not certain a path exists, find it with a search first — never construct paths from memory (the real entry may be `src/cli.tsx` while you guessed `src/cli.ts`).");
