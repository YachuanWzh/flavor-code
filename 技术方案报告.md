...
组合根集中在 `createProductionRuntime()`，业务模块之间以窄接口连接。`LocalHarness` 为主 Agent 和每个子 Agent 创建独立 `ContextManager`/`ToolRuntime`；`AgentLoop` 不读取配置文件，也不直接操作 UI；`SessionStore` 不接收 provider 配置，从类型边界减少凭据落盘风险。
...
sequenceDiagram
  ...
  alt 普通工具调用
    ...
    par 独立 DAG 节点
      A->>H: createSubagent(no Task, workspace mode)
      H->>M: cheaper model
      M-->>A: strict SubagentResult
    end
    A-->>T: ordered states/results
  end
  L-->>C: text/tool/usage/done events
  P->>S: persist stable lifecycle boundary
```

启动先加载全局/项目配置，再注册 provider 和插件贡献，发现 Skill，最后构造 Harness。只有传入 `resumeSession` 才读取会话。主循环在每轮模型请求前检查取消与压缩；工具调用整轮暂存后一次性追加 assistant/tool 消息，保证 provider 对话结构有效。
...
| `src/agent/loop.ts` | 最大迭代、模型流、工具整轮、usage/error 事件 |
| `src/context/manager.ts` | 当前固定指令、对话、工具输出截断、压缩、snapshot/restore |
...
| `src/hooks/bus.ts`, `types.ts` | v1 事件/决定 schema，超时和失败策略 |
| `src/agent/planner.ts`, `subagents.ts` | 严格 DAG、并发调度、结构化子 Agent 结果 |
| `src/skills/registry.ts`, `tool.ts` | Skill 发现/匹配/正文及资源的能力式读取 |
| `src/plugins/host.ts`, `types.ts` | manifest、优先级、激活/卸载、窄注册 API |
| `src/session/store.ts` | v1 schema、原子保存、受限读取、隔离与恢复归一化 |

## 5. 配置模型与优先级

`loadConfig()` 的深合并顺序为：`~/.flavor-code/flavor.json` < `<workspace>/.flavor/flavor.json` < `LoadConfigOptions.cli`。`${VAR}` 插值环境由进程环境再叠加项目 `.env`，因此 `.env` 同名值优先。最终统一通过 `FlavorConfigSchema`：provider 可声明 `type/baseURL/apiKey/defaultModel/cheapModel`；Agent 选择为 `provider:model`；`maxSubagents` 范围 1–16；权限默认为 `workspace`；上下文阈值必须为正整数。
...
`LocalHarness` 保存 `mainModelId/subagentModelId/permissionMode`，为每个 profile 创建独立 runtime/context。主 Agent 可动态切模型和权限；子 Agent 永久 `workspace` 且工具集合过滤 `Task`。所有 child 在成功、失败和取消路径都由 `dispose()` 收束。

`AgentLoop.run()` 默认最多 20 次迭代。它先追加 user 消息，再压缩，调用模型，累计 usage；工具调用通过 `ToolRuntime.execute()`，结果序列化为 tool 消息。取消、provider 终止不完整、序列化失败和迭代耗尽均产生结构化错误。

`ContextManager` 每次请求重新拼装当前 system、当前 `FLAVOR.md`、可选 task state、摘要和对话。字符数/4 只是文档化估算。压缩保留最近完整 user turn，经过 `PreCompact/PostCompact`，仅在全部成功后提交。`snapshot()` 不含固定层；`restore()` 丢弃 system 注入、孤立 tool 和无结果空 tool-call，从而保留当前固定指令并恢复 provider-valid turns。
...
事件必须为 `{version:1,type,payload}`；决定为 allow/deny/ask，加可选 reason、updatedInput、additionalContext。`HookBus` 校验 schema，按优先级/注册顺序运行，支持 handler timeout 与 error/allow/deny/ask failurePolicy。只有消费方明确支持时 updatedInput 才改变输入，例如 `TaskPlanner.plan()` 要求 `BeforePlan.updatedInput.graph` 并重新验证 DAG；决定对象不是任意代码执行许可。

## 11. Task DAG 与子 Agent 结果
...
子 Agent 最多返回两次：第一次不符合 `SubagentResultSchema` 时给修复提示，第二次仍非法则合成 failed。结果字段为 `taskId/status/summary/filesChanged/commandsRun/verification/artifacts/risks/suggestedNextSteps`；不保存隐藏推理或完整 transcript。调度取消会 abort 并等待所有已启动 child 的收尾 Hook。
...
恢复仅由 `--resume [session-id]` 或 `ProductionRuntimeOptions.resumeSession` 触发。工作区不一致直接拒绝。当前 system/FLAVOR 由新 ContextManager 生成；只注入恢复的摘要/规范消息。遗留 running 节点：依赖失败/blocked 则 failed，否则 pending。生产组合根在 prompt/tool 轮完成、任务转换/结果、压缩、模型/权限变化、SessionEnd 与 dispose 边界排队保存，避免流 token 级写盘。
...
Ink `App` 管理输出行、输入历史、审批和 Ctrl+C：第一次中断活动请求，后续退出。`FlavorSession` 串行化 submit，平衡 SessionStart/UserPromptSubmit/Stop/SessionEnd Hook，并路由内置/插件命令。`/clear` 仅清 UI 展示，不删除 ContextManager 或会话文件。
...
| 工具 | schema、权限、超时、取消、序列化 | ToolResult error + failure Hook |
...
测试使用 Vitest，按 config/models/context/agent/tools/permissions/hooks/plugins/skills/init/ui/cli/session 分层。所有测试无真实 API 凭据；provider 使用假 adapter 或不可达本地地址验证选择/错误。TypeScript 开启严格与 exact optional 属性检查。
