...
会话文件采用 JSONL 格式（`.jsonl` 扩展名）：首行为 `__meta` 标记的元数据行（version、sessionId、createdAt/updatedAt、workspace、models、permissionMode、tasks、可选的 conversation.summary），后续每行一条对话消息（role/content/toolCallId/toolCalls）。与旧版单行 JSON 的兼容读取保留在 `#parseSession()` 中，仅用于恢复存量会话；新保存全部使用 JSONL。
