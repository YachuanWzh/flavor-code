...
循环先追加一次 user 消息，将可选的 `additionalContext` 作为当轮 system 消息注入而不持久化，再调用 `prepareForModelCall()`。模型 usage 中的 input tokens 回写 ContextManager，下一轮优先用真实 usage 与保守估算的较大值判断压力。普通请求返回 `context_overflow` 时，循环在同一 iteration 内强制完整压缩并重建请求，最多重试一次；不会再次追加 user 消息或重放工具。工具调用仍通过 `ToolRuntime.execute()`，整个 assistant 消息与所有 tool 消息一次性 `appendMany()` 提交，保证 provider 对话结构有效。
...
完整压缩按安全 user/API 回合选择保留区，目标至少 10K token 或 5 条文本消息且不超过 40K token，不切断 tool call/result。`summarizeWithModel()` 使用当前角色模型、空工具集和 Claude Code 风格九段提示词；响应中的 `<analysis>` 草稿被剥离，只保存 `<summary>`。摘要请求自身发生 context overflow 时，每次移除最旧完整 API 回合，最多 3 次。PreCompact、摘要与 PostCompact 全部成功后才原子提交新边界；取消或失败保持原状态。自动完整压缩连续失败 3 次后熔断，手动 `/compact` 和响应式压缩绕过熔断。
