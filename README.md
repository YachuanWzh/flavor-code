...
侧栏的 **D2C** 模块支持从设计稿到可运行代码的完整闭环：导入 Pixso 导出的 HTML 目录并选择目标框架（Vue 3 / React），把生成任务投递给当前会话，Agent 依据 `d2c-pixso` 技能（SOP）在 `src/d2c-output/<task>/` 生成实现；随后自动启动 Vite dev server 做像素级对比，给出视觉还原度评分与结构化差异报告（区域偏移、色值偏差、字体差异）。结果工作台提供叠加/拉帘/闪烁/热力图等对比模式、SVG 标注层与按严重度排序的问题列表，可对差异逐条通过或退回并触发模块级修复；视觉审阅通过后可导入 Swagger/OpenAPI 文档，自动生成 Axios 封装与 Express mock server，进入接口联调与可交互验收阶段。
