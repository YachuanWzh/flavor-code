...
### 修复
- 修复 CLI 输出的文件路径过长、换行后无法通过 Ctrl+点击（macOS 上为 Command+点击）打开的问题：Markdown 链接、行内代码和正文中的文件路径现在附带完整的终端超链接，换行后的各行仍指向同一文件；相对路径按当前工作区解析，带空格的路径正确编码，并保留行号引用。

### 测试与维护
- 新增长路径换行回归测试，覆盖 Markdown 链接、行内代码、普通文本、加粗文本、带空格的绝对路径及行号；相关 17 项测试与 TypeScript 类型检查通过。
- `package.json` 与 `package-lock.json` 的项目版本更新为 `1.4.4-beta.3`。
...
- `/loop` 与 `/goal` 的 worker 达到单段 300 次迭代上限时不再把整个长程任务判为失败：已完成的工具效果会作为待验证检查点持久化，由宿主验证决定成功或携带差距进入下一执行段。
- 普通 turn 达到迭代、输出 token 或上下文边界时会写入持久化自动续跑；若同时触发堆轮换，续跑保持未领取状态并由新进程恢复，避免刷新后停在输入提示符。
- 取消生产 `/loop` 的 cycle/token 人工确认与 `/goal` 的轮数/重复 gap 数字终止：这些配置仅作为兼容或遥测检查点，长任务持续到宿主验证通过、用户显式取消，或遇到认证失败、模型不存在、验证环境不可用等继续重试无法解决的外部阻塞。限流、断流、输出/上下文限制等可恢复 worker 错误改为执行段边界。
- 长任务持久化状态只保留最近 64 个 cycle、verification 与 budget approval 明细，总计数及当前检查点继续单独累计，避免运行数日后状态数组、JSON 序列化和状态文件持续增长。
- 每条宿主验证命令落盘的 stdout/stderr 各自限制为最近 1KB，并显式标记截断；避免高日志测试在单轮内撑破 `/goal` 1MB 或 `/loop` 5MB 状态文件。
- 自动上下文压缩不再连续失败三次后永久熔断；prompt-too-long 恢复默认按二分裁减完整 API 轮次，直到压缩请求可进入模型窗口。
- `/goal` 的规划器与分类器遇到限流、断流或服务端错误时按有上限的指数退避持续重试；规划器的无效结构化响应也会重新规划。生产运行不再注入最大轮次/停滞次数。堆轮换 continuation 的有效期由 10 分钟延长到 24 小时，覆盖慢启动、主机重启与无人值守唤醒。
- CLI 子进程启用 1MB 间隔的稀疏 V8 allocation sampling；正常退出自动删除，内存轮换或异常退出保留在 `.flavor/tmp/*.heapprofile`，用于定位普查范围之外的真实分配调用栈。

### 测试与维护
- 新增长 turn 重复模型请求的取消监听回归，以及单 turn 250 次大工具输出的展示内存上限回归；生产压力场景验证 240 轮后完整 GC 堆保持稳定。
- 新增 `/loop`、`/goal` 迭代边界继续执行、普通 turn 迭代边界 durable queue 恢复，以及 launcher allocation profile 参数回归。
